        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center gap-10">
              {['Packages', 'Routes', 'Fleet', 'Reviews', 'Contact'].map(item => (
                <a key={item} href={`#${item.toLowerCase()}`} className={`text-[13px] font-medium hover:text-gold-500 transition-colors ${isScrolled ? 'text-stone-700' : 'text-white/80'}`}>{item}</a>
              ))}
            </div>
      ...
      <div className={`fixed inset-y-0 right-0 w-[320px] max-w-[85vw] bg-stone-50 z-[120] shadow-2xl transition-transform duration-500 ${isDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8">
          ...
          <nav className="flex flex-col gap-1">
            {['Packages', 'Routes', 'Fleet', 'Reviews', 'Contact'].map(item => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={toggleDrawer} className="px-4 py-3.5 rounded-xl text-[15px] font-medium text-stone-700 hover:bg-gold-500/10 hover:text-gold-600 transition-all flex items-center gap-3">
                {item}
              </a>
            ))}
            {localStorage.getItem('user') && (
